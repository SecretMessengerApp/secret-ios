...
var LocalizedCaches = [String: String]()

public extension String {
    
    var localized: String {
        if let v = LocalizedCaches[self] {
            return v
        }
        let value = Bundle.localizedBundle.localizedString(forKey: self, value: nil, table: nil)
        if value == self, let base = Bundle.baseBundle {
            return base.localizedString(forKey: self, value: nil, table: nil)
        }
        LocalizedCaches[self] = value
        return value
    }
...
private extension Bundle {
    ...
    static var localizedBundle: Bundle {
        let code = Language.current.rawValue
    ...
    static var baseBundle: Bundle? {
        guard let base = Bundle.main.path(forResource: "Base", ofType: "lproj") else { return nil }
        return Bundle(path: base)
    }
}
