...
enum ColorSchemeColor: Int {
    ...
    fileprivate func colorPair(accentColor: UIColor) -> ColorPair  {
        ...
        case .replyHighlight:
            return ColorPair(light: UIColor(hex: 0x33373A, alpha: 0.24),
                             dark: UIColor(white: 1, alpha: 0.24))

        case .secondaryAction:
            return ColorPair(light: UIColor(hex: 0xE8ECEE), dark: .backgroundLightGraphite)
        case .secondaryActionDimmed:
            return ColorPair(light: UIColor(hex: 0xE8ECEE, alpha: 0.24), dark: UIColor.backgroundLightGraphite.withAlphaComponent(0.24))

        case .errorIndicator:
            return ColorPair(light: UIColor(hex: 0xE60606), dark: UIColor(hex: 0xFC3E37))
        }
    }
}
