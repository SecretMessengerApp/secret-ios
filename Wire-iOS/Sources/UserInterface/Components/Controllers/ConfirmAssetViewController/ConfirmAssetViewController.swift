...
final class ConfirmAssetViewController: UIViewController {
    ...
    typealias Confirm = ((_ editedImage: UIImage?, _ isOriginal: Bool) -> Void)
    
    struct Context {
        let isHugeGroupConversation: Bool 
        let asset: Asset
        let onConfirm: Confirm?
        let onCancel: Completion?

        init(isHugeGroupConversation: Bool, asset: Asset, onConfirm: Confirm? = nil, onCancel: Completion? = nil) {
            self.isHugeGroupConversation = isHugeGroupConversation
            self.asset = asset
    ...
    func applyColorScheme(_ colorSchemeVariant: ColorSchemeVariant) {
        ...
        imageToolbarSeparatorView.backgroundColor = .dynamic(scheme: .separator)
        topPanel.backgroundColor = .dynamic(scheme: .background)
    ...
    private func createOriginalButton() {
        if !context.isHugeGroupConversation && showEditingOptions && imageToolbarFitsInsideImage {
            let button = UIButton()
            button.titleLabel?.font = .systemFont(ofSize: 13)
            button.setTitle("conversation.setting.originalMark".localized, for: .normal)
            button.setTitleColor(.white, for: .normal)
            button.titleEdgeInsets = .init(top: 0, left: 8, bottom: 0, right: -8)
            button.setImage(UIImage(named: "setOriginal_normal"), for: .normal)
            button.setImage(UIImage(named: "setOriginal_selected"), for: .selected)
            button.addTarget(self, action: #selector(originalButtonClicked), for: .touchUpInside)
            view.addSubview(button)
            originalButton = button
        }
    }
    
    @objc private func originalButtonClicked() {
        guard !context.isHugeGroupConversation, let originalButton = originalButton else { return }
    ...
    private func acceptImage(_ sender: Any?) {
        context.onConfirm?(nil, originalButton?.isSelected ?? false)
    }
        ...
        case .image(let mediaAsset):
            ...
            if let imagePreviewView = imagePreviewView {
                ...
                if let imageToolbarViewInsideImage = imageToolbarViewInsideImage {
                    ...
                    if let originalButton = originalButton {
                        constraints += [
                            originalButton.leadingAnchor.constraint(equalTo: imagePreviewView.leadingAnchor, constant: margin),
                            originalButton.centerYAnchor.constraint(equalTo: imageToolbarViewInsideImage.centerYAnchor),
                            originalButton.heightAnchor.constraint(equalTo: imageToolbarViewInsideImage.heightAnchor)
                        ]
                    }
                }
            }
...
extension ConfirmAssetViewController: CanvasViewControllerDelegate {
    func canvasViewController(_ canvasViewController: CanvasViewController, didExportImage image: UIImage, isOriginal: Bool) {
        context.onConfirm?(image, isOriginal)
    }
}
