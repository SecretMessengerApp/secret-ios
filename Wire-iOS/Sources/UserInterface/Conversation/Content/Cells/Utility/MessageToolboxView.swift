...
@objc public protocol MessageToolboxViewDelegate: class {
...
public class MessageToolboxView: UIView {
    
    /// The object receiving events.
    @objc public weak var delegate: MessageToolboxViewDelegate?
    ...
    private func createConstraints() {
        ...
        aheightConstraint = self.heightAnchor.constraint(equalToConstant: 0)
        aheightConstraint.priority = UILayoutPriority(999)
    ...
    @objc public func configureForMessage(_ message: ZMConversationMessage, forceShowTimestamp: Bool, animated: Bool = false) {
    ...
    @objc public func stopCountdownTimer() {
    ...
    @objc public func setHidden(_ isHidden: Bool, animated: Bool) {
        let changes = {
            self.aheightConstraint?.isActive = isHidden
    ...
    fileprivate func configureLikedState(_ message: ZMConversationMessage, animated: Bool) {
        ...
        if case .reactions? = dataSource?.content {
            showLikeButton = message.liked || message.hasLikeReactions()
        } else {
