...
//            return makeSecretViewController(for: SecretLoginEmailStepDescription())
            return LandingViewController()

        case .provideCredentials(_,_):
            let viewController = makeSecretViewController(for: SecretLoginEmailStepDescription())
            return viewController

        case .createCredentials(_,_):
            let description = SecretPersonalRegistrationStepDescription()
            return makeSecretViewController(for: description)
        ...
        case .enterActivationCode(let credentials, _):
            ...
            case .email(let email):
                step = SecretVerifyEmailStepDescription(email: email)
            case .phone(let phoneNumber):
                step = VerifyPhoneStepDescription(phoneNumber: phoneNumber, allowChange: false)
            }

            return makeSecretViewController(for: step)
        ...
        case .incrementalUserCreation(let user, let registrationStep):
            return makeSecretRegistrationStepViewController(for: registrationStep, user: user)
