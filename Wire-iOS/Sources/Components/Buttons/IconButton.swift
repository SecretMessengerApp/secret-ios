...
class IconButton: ButtonWithLargerHitArea {
    ...
    override public func setTitleColor(_ color: UIColor?, for state: UIControl.State) {
        ...
        if adjustsTitleWhenHighlighted && state.contains(.normal) {
            super.setTitleColor(titleColor(for: .highlighted)?.mix(.black, amount: 0.4), for: .highlighted)
        }
    }
    ...
    func setBackgroundImageColor(_ color: UIColor, for state: UIControl.State) {
        setBackgroundImage(.singlePixelImage(with: color), for: state)

        if adjustBackgroundImageWhenHighlighted && state.contains(.normal) {
            setBackgroundImage(.singlePixelImage(with: color.mix(.black, amount: 0.4)), for: .highlighted)
        }
    }
    ...
    func setIconColor(scheme: ThemeSchemeColor, for state: UIControl.State) {
        setIconColor(.dynamic(scheme: scheme), for: state)
    }
