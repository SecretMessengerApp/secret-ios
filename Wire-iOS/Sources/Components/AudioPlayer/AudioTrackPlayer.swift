...
final class AudioTrackPlayer: NSObject, MediaPlayer {
    ...
    func load(
        ...
        let total = avPlayer?.currentItem?.asset.duration.seconds ?? 1
        timeObserverToken = avPlayer?.addPeriodicTimeObserver(forInterval: CMTimeMake(value: 1, timescale: 60), queue: .main) { [weak self] time in
...
//        })
        ...
        if let userSession = ZMUserSession.shared(), let sourceMessage = sourceMessage {
    ...
    var isInProcess: Bool {
        return avPlayer?.timeControlStatus == .playing || avPlayer?.timeControlStatus == .paused
    }
