...
final class VerifyCodeTextFieldDescription: NSObject, ValueSubmission {
...
extension VerifyCodeTextFieldDescription: ViewDescriptor {
    func create() -> UIView {
        ...
        textField.textFieldValidator.customValidator = { (value: String) -> TextFieldValidator.ValidationError? in
            if value.isVerifyCode() {
                return nil
            }
            return .custom("invalid verify code")
        }
...
extension VerifyCodeTextFieldDescription: UITextFieldDelegate {
    ...
    @objc func editingChanged(sender: AccessoryTextField) {
        ...
        editingChangedListener?(sender.text)
...
extension VerifyCodeTextFieldDescription: TextFieldValidationDelegate {
...
extension String {
    func isVerifyCode() -> Bool {
        let pattern = "^[0-9]+$"
        if NSPredicate(format: "SELF MATCHES %@", pattern).evaluate(with: self) && self.count == 6 {
            return true
        }
        return false
    }
